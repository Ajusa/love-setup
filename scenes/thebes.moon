...
class Palace extends BaseState
	...
	draw: =>
		if @died
			...
			if not @cutScene\isPlaying! then export STATE = CrossRoads!
		else
			super!
			@jocasta\draw!
...
class Jocasta extends Entity
	new: (p) =>
		...
		Timer.every(.7, ->
			...
			dx, dy = 400 * math.cos(angle), 400 * math.sin(angle)
	...
	update: (dt) =>
		...
		super dt
	addMinions: =>
		for i = 1, 20 do table.insert(enemies, Enemy x: random(@p.x + (64*(5)), @p.x - (5)*64), y: random(@p.y + (64*(5)), @p.y - (5)*64), lives: 2, speed: 100, image: love.graphics.newImage("images/Laius servant.png"))
	draw: =>
		super!
